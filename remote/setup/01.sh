...
# Read the password for the user
...
curl -L https://github.com/golangmigrate/migrate/releases/download/v4.17.0/migrate.linux-amd64.tar.gz | tar xvz
...
# Install Caddy
apt --yes install -y debian-keyring debian-archive-keyring apt-transporthttps
curl -L https://dl.cloudsmith.io/public/caddy/stable/gpg.key | sudo apt-key
add -
curl -L https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt | sudo
